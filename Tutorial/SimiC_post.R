...
seurat_data <- readRDS('./Data/clonalKinetics_Example.rds')
...
weights_file <- './Data/ClonalKinetics_filtered_L10.01_L20.01_Ws.pickle'
SimiC_weights <- py_load_object(filename =weights_file)
pd <- import("pandas")
AUCs_file = './Data/ClonalKinetics_filtered_L10.01_L20.01_AUCs.pickle'
...
pdf(paste0('./Plots/Simic_Filtered_R2_targets_hist.pdf'))
for (phenotype in phenotypes){
...
pdf(paste0('./Plots/Simic_TF_weigths.pdf'), onefile = TRUE, width=20)
TF_2_remove <- c()
...
pdf(paste0('./Plots/Simic_Targets_weigths.pdf'), onefile = TRUE, width=20)
plot_counter <- 1
plot_list <- list()
...
pdf(paste0('./Plots/Simic_Population_phenotype.pdf'))
...
for (clust in clusters_2_keep){
  ...
  pdf(paste0('./Plots/Simic_Auc_Cluster_',clust,'.pdf'), width = 15, onefile = TRUE)
  plot_counter <- 1
...
p <- pheatmap(MinMax_clust,color=plasma, fontsize=5, angle_col =45, cellwidth=40, silent=TRUE)
plot_dims <- get_plot_dims(p)
pdf(paste0('./Plots/Simic_HeatMaps_RegDissScore.pdf'), height = plot_dims$height, width = plot_dims$width )
print(p)
dev.off()
...
pdf(paste0('./Plots/Simic_tSNE_selection.pdf'), height =8, width=18)
